...
class Spider:
    def __init__(self):
        ...
        self.path = 'C:\\Users\\16904\\Desktop\\JMusic.txt'
    ...
    def bubble_sort(lists):
        # print(len(lists)-1)
        for time1 in range(len(lists)):
            for time2 in range((len(lists))-time1-1):
    ...
    def txt_write(lists):
        ...
        '''for each in range(len(lists)-1):
            stren1 = '\nDelay st + {0} - TickCount()'.format(lists[each][0])
            if lists[each + 1][0] - lists[each][0] > 4:
                stren.append(stren1)
            if lists[each][3] == 'down':
                stren2 = '\nTouchDownEvent {0},{1}'.format(lists[each][1], lists[each][2])
            else:
    ...
    def eachsite(self):
        ...
        for oneline in file:                         # 遍历文本中每一个网址
            ...
            url = 'https://card.niconi.co.ni/live/Live_s0984.json'
            print(url)
            imfor = self.web_driver(url)               # 抓取源码
            result_title = self.re_title(imfor)         # 提取歌曲名
            logging.debug(result_title)
            dress = 'D://JM//{0}.txt'.format(result_title)   # 以歌曲名创建文本
            print(dress)
            logging.debug('文件路径 '+dress)
            ...
            # print('seconddata', line)
            bubble_lists = self.bubble_sort(data_list)                                                                        # 数据排序
            # for line in bubble_lists:
            # print('third', line)
            if bubble_lists == 0:
                print('wrong')
            data_list = []                      # 释放内存
...
spider = Spider()
'''SourcePage = spider.web_request(spider.web_url)
